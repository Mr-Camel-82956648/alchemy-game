...
1. 先启动后端（`backend` 目录，默认 `18001`）
2. 再在项目根目录启动静态服务（默认 `8000`）
3. 访问 `http://localhost:8000/`，会先进入 `alchemy_intro_book/`，再跳转到主游戏 A 的 `/frontend/`
...
- 当前新增 `frontend/tools/arena_glyph_tuner.html`，用于在 battle 外用共享 canvas 渲染链核查竞技场法阵视频的尺寸、位置、混合模式与近似后处理参数；当前正式默认参数也通过这条共享渲染链统一落到 battle 与 tuner，但它仍然只属于本地调试目录，不是正式业务上下文。
